...
class NapeDrawDebug
{
	...
	private var compoundList:CompoundList;
	...
	public function draw(graphics:Graphics):Void
	{
		...
		// COMPOUNDS
		for (i in 0...compoundList.length) {
			for (bs in 0...compoundList.at(i).bodies.length)
			{
				var body:Body = compoundList.at(i).bodies.at(bs);
				if (drawBodyBounds){
					graphics.color = bodyBoundsColor;
					graphics.drawRect(body.bounds.x, body.bounds.y, body.bounds.width, body.bounds.height);
				}
